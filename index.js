...
function getFullNames(runners) {
  ...
  runners.forEach(element => {
    let x = (`${element.last_name}, ${element.first_name}`);
    fullNames.push(x);
  })
...
// const capped = runners.map((namesCapped) => {
//   return {first_name : namesCapped.first_name};
//   namesCapped.push(x);
// });
// let myMap = new Map(runners)
// myMap.get('first_name')
// var x = runners.map()
// .toUpperCase();
// runners.first_name
return runners.map((element) => {
...
function tallyUpDonations(runners) {
  return runners.reduce((total,state) => total += state.donation, 0);
}
